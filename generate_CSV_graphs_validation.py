...
for i, instance_folder in enumerate(glob('Networks/Network*')):

    instance_info = instance_folder.split('/')[1].replace('Network', '')
    instance_seed = int(instance_info.split('_')[-1])
    ...
    instances_id_to_info[current_id] = instance_info
    ...
    df_nodes['graph_id'] = current_id
    df_edges['graph_id'] = current_id
    ...
    df_all_graphs.loc[len(df_all_graphs)] = [current_id, 0, current_id, nb_trip]
    ...
    print('{}/100 done'.format(current_id - 700))



with open('Networks/instances_id_to_info_validation.pkl', 'wb') as f:
...
df_all_graphs.to_csv('MDEVSP_VAL_dataset/graphs.csv', sep=',', index=False)
df_all_nodes.to_csv('MDEVSP_VAL_dataset/nodes.csv', sep=',', index=False)
df_all_edges.to_csv('MDEVSP_VAL_dataset/edges.csv', sep=',', index=False)
